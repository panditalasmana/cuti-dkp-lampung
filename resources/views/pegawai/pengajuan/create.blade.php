                        ...
                        <div class="row g-3">
                            <div class="col-sm-6">
                                ...
                                <select name="atasan_langsung_select" class="form-select @error('atasan_langsung_select') is-invalid @enderror" required>
                                    ...
                                    @if(isset($signers['atasan_langsung']) && count($signers['atasan_langsung']) > 0)
                                        @foreach($signers['atasan_langsung'] as $s)
                                            <option value="{{ $s->piped_value }}" {{ old('atasan_langsung_select', $s->is_default ? $s->piped_value : '') == $s->piped_value ? 'selected' : '' }}>
                                                {{ $s->nama }} ({{ $s->jabatan }} - NIP. {{ $s->nip }})
                                            </option>
                                        @endforeach
                                    @else
                                        <option value="A. FAISAL, A.Pi.|197402031999031006|Sekretaris Dinas" selected>
                            ...
                            <div class="col-sm-6">
                                ...
                                <select name="pejabat_wenang_select" class="form-select @error('pejabat_wenang_select') is-invalid @enderror" id="pejabatWenangSelect" required>
                                    ...
                                    @if(isset($signers['pejabat_wenang']) && count($signers['pejabat_wenang']) > 0)
                                        @foreach($signers['pejabat_wenang'] as $s)
                                            <option value="{{ $s->piped_value }}" {{ old('pejabat_wenang_select', $s->is_default ? $s->piped_value : '') == $s->piped_value ? 'selected' : '' }}>
                                                {{ $s->nama }} ({{ $s->jabatan }} - NIP. {{ $s->nip }})
                                            </option>
                                        @endforeach
                                    @else
                                        <option value="Ir. BANI ISPRIYANTO, M.M.|196904101995031002|Kepala Dinas" selected>
                            ...
                            <div class="col-12 mt-2">
                                <label class="form-label fw-semibold">Pejabat Pengawas <span class="text-muted small">(Opsional)</span></label>
                                <select name="eselon_3" class="form-select @error('eselon_3') is-invalid @enderror">
                                    <option value="">-- Pejabat Pengawas (Opsional) --</option>
                                    @php
                                        $pengawasItems = $signers['eselon_4'] ?? $signers['pejabat_pengawas'] ?? [];
                                    @endphp
                                    @if(count($pengawasItems) > 0)
                                        @foreach($pengawasItems as $s)
                                            <option value="{{ $s->piped_value }}" {{ old('eselon_3') == $s->piped_value ? 'selected' : '' }}>
                                                {{ $s->nama }} ({{ $s->jabatan }} - NIP. {{ $s->nip }})
                                            </option>
                                        @endforeach
                                    @endif
                ...
                <ul class="list-group list-group-flush">
                    @foreach($jenisCuti as $jc)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            @if($jc->potong_kuota)
                                <span class="badge bg-secondary text-white small">Kuota Tahunan</span>
                            @else
